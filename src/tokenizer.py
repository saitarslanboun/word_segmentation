...
def main_tokenizer():
    ...
    #file_name = os.getcwd() + "/" + f[:-4] + ".data"
    text_file = open("train.data", "w")
    ...
    def _tokenize(sentence, tokenizer):
	    ...
	    text_file.write('"')
	    token = token.replace('"', '*')
            text_file.write(token.encode('utf8'))
	    text_file.write('"')
	    sep = ' '

        #stdout.write(linesep)
	text_file.write(linesep)
	...
	#for txt_file_path in p.iterdir():
    with codecs.open("train.txt", 'r', encoding=(args.encoding or 'utf-8')) as fp:
    	for line in fp:
